...
export default function StaticRouteMap({ pickup, drop, routeData, isOnline }) {
    ...
    useEffect(() => {
        ...
        const updateMap = () => {
            ...
            if (!drop) {
                ...
                if (isOnline) {
                    radarHtml = `
                        <div class="absolute top-[45px] left-1/2 -translate-x-1/2 -translate-y-1/2 w-32 h-32 rounded-full animate-[ping_3s_cubic-bezier(0,0,0.2,1)_infinite] pointer-events-none" style="background: hsl(169,59%,31%,0.15)"></div>
                        <div class="absolute top-[45px] left-1/2 -translate-x-1/2 -translate-y-1/2 w-8 h-8 rounded-full animate-pulse pointer-events-none" style="background: color-mix(in srgb, var(--clr-primary) 20%, transparent)"></div>
                    `;
                }
                ...
                el.innerHTML = `
                    <div class="relative z-10 flex flex-col items-center drop-shadow-xl">
                        <svg width="34" height="50" viewBox="0 0 27 41" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M13.5 0C6.04416 0 0 6.04416 0 13.5C0 23.625 13.5 41 13.5 41C13.5 41 27 23.625 27 13.5C27 6.04416 20.9558 0 13.5 0Z" fill="var(--clr-foreground)"/>
            ...
            if (routeData) {
                ...
                startEl.innerHTML = `
                    <div class="text-white px-3 py-2 text-center text-sm font-bold leading-tight" style="background: var(--clr-foreground)">
                        ${durationMins}<br/>min
                    </div>
    ...
    return (
        <div className="absolute inset-0 w-full h-full">
            ...
            {!isMapLoaded && (
                <div className="absolute inset-0 z-50 flex items-center justify-center" style={{ background: 'var(--clr-bg)' }}>
                    <div className="flex flex-col items-center gap-3">
                        <div className="w-9 h-9 rounded-full border-[3px] animate-spin" style={{ borderColor: 'color-mix(in srgb, var(--clr-primary) 20%, transparent)', borderTopColor: 'var(--clr-primary)' }} />
                        <span className="text-[11px] font-bold uppercase tracking-widest" style={{ color: 'hsl(169,59%,31%,0.7)' }}>Loading Map</span>
                    </div>
                </div>
            )}
        </div>
